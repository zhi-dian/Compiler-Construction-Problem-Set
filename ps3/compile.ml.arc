...
let rec compile_stmt ((s,_):Ast.stmt) : inst list = 
    (*************************************************************)
    let castfalse = new_label() in
    let endcast = new_label() in
    let cast = new_label() in
    let int2bool = [Label(cast);Beq(R2,R0,castfalse);Li(R2,Int32.one);J(endcast);Label(castfalse);
    Li(R2,Int32.zero);Label(endcast);Jr(R31)] in
    int2bool @ 
    (let rec compile_stmt' ((s',_):Ast.stmt) : inst list = 
        let rec compile_exp ((e,_):Ast.exp) : inst list = 
            ...
            | And(e1,e2) -> 
                let t = new_temp()in(compile_exp e1) @ [La(R3,t); Jal(cast); Sw(R2,R3,Int32.zero)]
                @(compile_exp e2) @ [La(R3,t); Lw(R3,R3,Int32.zero)]
                @[Jal(cast);And(R2,R2,Reg R3)]
            | Or(e1,e2) -> 
                let t = new_temp()in(compile_exp e1) @ [La(R3,t); Jal(cast); Sw(R2,R3,Int32.zero)]
                @(compile_exp e2) @ [La(R3,t); Lw(R3,R3,Int32.zero)]
                @[Jal(cast);Or(R2,R2,Reg R3)]
            | Binop(e1,b,e2) ->
                let t = new_temp()in
            ...
            | Not e1 -> (compile_exp e1)@[Jal(cast);Xor(R2,R2,Immed Int32.one)])
        in
        (match s' with 
        | Exp e ->
            compile_exp e
