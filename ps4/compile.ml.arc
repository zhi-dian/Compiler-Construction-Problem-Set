...
(* create a map for function env size and variables' offset *)
let creat_env_fun name args stmt : unit = 
...
let rec create_env (p: Ast.program) : unit = 
    ...
    | fn::rest -> (match fn with
        | Fn {name=n; args=a; body=s; pos=p} -> 
            ((creat_env_fun n a s);(create_env rest)))
...
let rec compile_stmt ((s,_):Ast.stmt) (name:string) : inst list = 
  ...
  let rec compile_exp ((e,_):Ast.exp) (name: string): inst list = 
      ...
      | Call(funname,elist) -> 
        ...
        (compile_arg elist) @
        [
        (* Sw(R30,R30,((Int32.of_int (EnvMap.find (name^"30") (!env)))));
        Sw(R31,R30,(Int32.of_int (EnvMap.find (name^"31") (!env)))); *)
        Add(R30,R29,(Immed (Int32.of_int (cnt_args(elist)-1))));
        Add(R29,R30,(Immed (Int32.sub 1l (Int32.of_int (EnvMap.find funname (!env))))))]
         @ [Jal funname;
         Lw(R30,R30,((Int32.of_int (EnvMap.find (name^"30") (!env)))));
         Lw(R31,R30,(Int32.of_int (EnvMap.find (name^"31") (!env))));
          
         (* Add(R29,R29,(Immed (Int32.of_int (EnvMap.find funname (!env))))) *)
         ]
  in
  match s with 
  | Exp e ->
  ...
  | Return(e) -> (compile_exp e name)@[
    Add(R29,R29,Immed (Int32.of_int (EnvMap.find name (!env))))]@ if name = "main" then [Lw(R31,R29,0l);J("printInt")] else 
      (* @[Sw(R2,R2,Int32.zero)] *)
       [Jr(R31)]
      (* @[Jr(R31)] *)
  (* | Let(id,exp,s) -> [] *)
    ...
    | fn::rest -> (match fn with
     | Fn {name=n; args=a; body=s; pos=_} -> ([Label n] @
      (if n="main" then 
        ...
        Add(R30,R29, Immed 7l)] else [])
      @
    [Sw(R31,R30,(Int32.of_int (EnvMap.find (n^"31") (!env))));
     Sw(R30,R30,(Int32.of_int (EnvMap.find (n^"30") (!env))));] @ compile_stmt s n)@
    (* [Add(R29,R29,Immed (Int32.of_int (EnvMap.find n (!env)))); Jr(R31)]@ *)
    (compile' rest) )in
