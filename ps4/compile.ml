...
let rec compile_stmt ((s,_):Ast.stmt) (name:string) : inst list = 
  ...
  let rec compile_exp ((e,_):Ast.exp) (name: string): inst list = 
      ...
      | Call(funname,elist) -> 
        ...
        (compile_arg elist) @
        [
        Add(R30,R30,(Immed (Int32.sub 0l (Int32.of_int (EnvMap.find name (!env))))));
        Add(R29,R30,(Immed (Int32.sub 1l (Int32.of_int (EnvMap.find funname (!env))))))]
         @ [Jal funname;
         Add(R30,R30,(Immed ((Int32.of_int (EnvMap.find name (!env))))));
         Lw(R31,R30,(Int32.of_int (EnvMap.find (funname^"31") (!env))));
  ...
  | Return(e) -> (compile_exp e name)@[
    Add(R29,R29,Immed (Int32.of_int (EnvMap.find name (!env))))]@ if name = "main" then [Add(R4,R2,Immed 0l);J("printInt")] else 
      (* @[Sw(R2,R2,Int32.zero)] *)
       [Jr(R31)]
      (* @[Jr(R31)] *)
  (* | Let(id,exp,s) -> [] *)
...
let rec compile (p:Ast.program) : result =
    ...
    | [] -> []
    | fn::rest -> (match fn with
     | Fn {name=n; args=a; body=s; pos=_} -> ([Label n;] @ compile_stmt s n)@
