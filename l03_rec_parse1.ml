...
struct
  ...
  (* document -- 
     zero or more tokens separated by one or more white spaces.  This
     is actually not quite right.  For instance, we need white space
     between identifiers, but we don't need whitespace between other
     kinds of tokens.  *)
  ...
  let rec exp_p : (token,exp) grammar = 
    fun s -> 
      alts [
       ...
       (* ;
          (* exp '+' exp *)
          (exp_p $ tok PLUS $ exp_p) %
          (function ((e1,_),e2) -> Binop(e1,Plus,e2)) ; *)
      ] s
